#ifndef CPP2_S21_CONTAINERS_1_S21_RED_BLACK_FORESTS_TPP_
#define CPP2_S21_CONTAINERS_1_S21_RED_BLACK_FORESTS_TPP_
...
//    new_node->pair = std::pair<Key, T>(key, value);
  ...
  void red_black_tree<Key,T>::merge(red_black_tree<Key,T> &other) {
    if (other.root_ != nullptr) {
      ...
      while (it.current_ && it.current_ != it.conductor) {
  ...
  size_t red_black_tree<Key,T>::size() {
    size_t count = 0;
    int stop = 0;
    iterator it = this->tree_begin();
    if (it.current_) {
      while (it.current_ && stop < 30 && it.current_ != it.conductor) {
//      std::cout << it.current_->key << std::endl;
        stop++;
        count++;
        it++;
      }
...
#endif // CPP2_S21_CONTAINERS_1_S21_RED_BLACK_FORESTS_TPP_