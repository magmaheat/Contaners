#ifndef CPP2_S21_CONTAINERS_SRC_INCLUDE_S21_MAP_H_
#define CPP2_S21_CONTAINERS_SRC_INCLUDE_S21_MAP_H_
...
class map : public red_black_tree<Key, T> {
 public:
  ...
  map(const map& other) : red_black_tree<Key, T>() { this->add(other.root); }
  ...
  class map_iterator : public tree::tree_iterator {
   public:
    ...
    std::pair<const Key, T>* operator->() { return &this->current->pair; }
  ...
  std::pair<iterator, bool> insert(const value_type& value) {
    std::pair<typename red_black_tree<Key, T>::Node*, bool> return_insert =
        this->insert_local(value.first, value.second, 3);
    return std::pair<iterator, bool>(iterator(return_insert.first, this),
                                     return_insert.second);
  }

  std::pair<iterator, bool> insert(const key_type& key,
                                   const mapped_type& data) {
    std::pair<typename red_black_tree<Key, T>::Node*, bool> return_insert =
        this->insert_local(key, data, 3);
    return std::pair<iterator, bool>(iterator(return_insert.first, this),
                                     return_insert.second);
  }
  ...
  T& at(const key_type& key) { return this->find_local(key)->data; }
...
#endif  // CPP2_S21_CONTAINERS_SRC_INCLUDE_S21_MAP_H_